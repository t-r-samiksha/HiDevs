...
// ---------------------------------------------------------------------------
// Signal 1: Decision Velocity — detects a >40% drop in decisions this week
// ---------------------------------------------------------------------------
async function decisionVelocity(project_id: string): Promise<Signal | null> {
  ...
  const countInRange = (from: number, to: number) =>
    data.filter((d) => {
      const age = now - new Date(d.created_at).getTime();
      return age >= from && age < to;
    }).length;

  const w0 = countInRange(0, wk);
  const w1 = countInRange(wk, 2 * wk);
  const w2 = countInRange(2 * wk, 3 * wk);
...
// ---------------------------------------------------------------------------
// Signal 2: Recurring Blocker Clusters — >3 dependency hints with >0.8 similarity
// ---------------------------------------------------------------------------
async function recurringBlockerClusters(project_id: string): Promise<Signal | null> {
  ...
  return {
    ...
    description: `${clusterCount} dependency hint(s) cluster with >80% semantic similarity. Common theme: "${commonTheme.slice(0, 80)}".`,
    severity: "warning",
...
// ---------------------------------------------------------------------------
// Signal 3: Commitment Drift — items whose deadlines have been shifted repeatedly
// Approximation: open items with deadline that have been touched 3+ days after creation
// ---------------------------------------------------------------------------
async function commitmentDrift(project_id: string): Promise<Signal | null> {
  ...
  const drifted = data.filter((item) => {
    const createdMs = new Date(item.created_at).getTime();
    const updatedMs = new Date(item.updated_at).getTime();
    return updatedMs - createdMs > 3 * 24 * 60 * 60 * 1000;
  });
...
// ---------------------------------------------------------------------------
// Signal 4: Meeting ROI — meetings in the last 2 weeks with zero output
// ---------------------------------------------------------------------------
async function meetingROI(project_id: string): Promise<Signal | null> {
  ...
  const itemsByMeeting = new Set((items || []).map((i) => i.meeting_id));
  const zeroROI = meetings.filter((m) => !itemsByMeeting.has(m.id));
...
// ---------------------------------------------------------------------------
// Signal 5: Cross-Project Opportunity — similar decisions in other projects
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Workflow step: run all 5 signal functions concurrently
// ---------------------------------------------------------------------------
const gatherInsightsStep = createStep({
  id: "gather-insights",
  description: "Run all 5 strategic signal detectors concurrently and return non-null signals",
  ...
  execute: async ({ inputData }) => {
    ...
    for (const result of results) {
      if (result.status === "fulfilled" && result.value !== null) {
        ...
        console.error("Strategic insight signal failed:", result.reason);
      }
    }
...
export const strategicInsightWorkflow = createWorkflow({
  ...
  name: "Strategic Insight Workflow",
  description:
    "Runs 5 strategic signal detectors concurrently: decision velocity drop, " +
    "recurring blocker clusters, commitment drift, meeting ROI, and cross-project " +
    "opportunity. Returns only non-null signals ranked by severity.",
  inputSchema: z.object({ project_id: z.string() }),
  outputSchema: z.object({ signals: z.array(SignalSchema) }),
})
  .then(gatherInsightsStep);
